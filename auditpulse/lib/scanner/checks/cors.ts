    ...
    async run(ctx): Promise<Finding[]> {
        ...
        if (allowOrigin === '*' && allowCredentials?.toLowerCase() === 'true') {
            findings.push({
                ...
                severity: 'critical',
                impact: 'Any website a logged-in visitor happens to open — even an unrelated, malicious one — could silently read their private data from your site in the background, using their own active login session.',
            ...
            findings.push({
                ...
                impact: 'Any website on the internet can pull data straight out of this endpoint using a visitor\'s browser as the messenger, bypassing the protection that\'s supposed to stop other sites from doing exactly that.',
