...
- **Ads** (`#ads`) is a self-serve ad slot: anyone can pitch a one-line headline and a link, pick a view package (1,000 / 5,000 / 20,000 views), and pay through Stripe Checkout. A paid ad lands `pending_review`; approve it from the admin **Ads** tab to start rotating it in the sponsored slot on the homepage. It stops rotating on its own once its purchased views are used up (tracked via `/api/directory?action=ad-impression`, called once per ad shown per visitor session). Requires `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` for checkout/activation and `ADMIN_SECRET` for the review queue — see below.
